...
export default function FormEdit() {
  ...
  const getTicket = () => {
    const id = dataEdit.tiket_keluhan;
    kelFunc({
      axiosInstance: axios,
      method: "GET",
      url: `ticket-select2/${id}`,
      data: null,
      reqConfig: {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("auth")}`,
  ...
  const loadEdit = () => {
    for (const key in dataEdit) {
      if(key === 'tiket_keluhan') {
        dispatch({
          type: "CHANGE_INPUT",
          payload: {
            name: "_ticket_keluhan",
            value: dataEdit[key],
          },
        });
      } else {
  ...
  useEffect(() => {
    const inv =
      teknisi.length > 0 &&
      setTimeout(() => {
        const selected = teknisi.filter(
          ({ value }) => value == dataEdit.pegawai_id
        );
        setTeknisi(selected[0]);
      }, 1);
    return () => clearInterval(inv);
  }, [teknisi]);
  ...
  useEffect(() => {
    const inv =
      keluhan.length > 0 &&
      setTimeout(() => {
        const selected = keluhan.filter(
          ({ value }) => value == dataEdit.tiket_keluhan
        );
        setKeluhan(selected[0]);
      }, 1);
    return () => clearInterval(inv);
  }, [keluhan]);
  ...
  return (
    <div className="row col-12 col-xl-6 bg-light rounded mx-0">
      <div className="d-flex justify-content-between align-items-center py-3 border-bottom">
        <h3 className="mb-0">Ubah Keluhan</h3>
